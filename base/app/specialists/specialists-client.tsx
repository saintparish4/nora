...
export default function SpecialistsClient({ initialData }: SpecialistsClientProps) {
  const { data, error, isLoading } = useSWR('providers-list', providersFetcher, {
    ...
    dedupingInterval: 60_000, // avoid refetching within 60s
  });
  ...
  // Only show loading state when there is no data at all (first client-only visit with no SSR data)
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {error && !loading && (
              <div className="col-span-full rounded-xl border border-[var(--glass-border)] bg-[rgba(255,255,255,0.4)] p-8 text-center">
                ...
                <p className="text-sm opacity-60">{error instanceof Error ? error.message : 'Failed to load specialists'}</p>
              </div>
            )}
